...
class ForecastRequest(BaseModel):
    ...
    temperature: Optional[List[float]] = Field(
        None, description='Future Temperature_Celsius per month'
    )
    humidity: Optional[List[float]] = Field(
        None, description='Future Relative_Humidity_percent per month'
    )
    solar_radiation: Optional[List[float]] = Field(
        None, description='Future Solar_Radiation_MJ_m2_per_month per month'
    )
    pressure: Optional[List[float]] = Field(
        None, description='Future Surface_Pressure_hPa per month'
    )
