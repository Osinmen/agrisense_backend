...
def predict_rainfall(request: ForecastRequest):
    ...
    try:
        last_date = climatology_service.get_last_date()
        future_dates = generate_future_dates(last_date, request.n_months)

        exog_list = build_exog_list(
            future_dates=future_dates,
            temperature=request.temperature,
            humidity=request.humidity,
            specific_humidity=request.specific_humidity,
        ...
        exog_source = (
            ...
            if any([request.temperature, request.humidity,
                    request.specific_humidity, request.solar_radiation,
                    request.pressure])
            else 'climatology'
        )
